...
if(isset($_POST['sub'])){
	...
	$attendant=mysqli_real_escape_string($con,$_POST['attendant']);
	   ...
	   if($location==''&& $street==''&& $name=='' && $slot=='' && $price=='' && $remaining_slots==''){
		...
		$insert="INSERT INTO `parkings` (`id`, `location`, `street`, `name`, `slot`,`remaining_slots`,`attendant`, `price` ) VALUES (NULL, '$location', '$street', '$name', '$slot' ,'$remaining_slots','$attendant', '$price');";
